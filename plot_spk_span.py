...
with plt.style.context(('seaborn')):
    ...
    plt.loglog(np.arange(1,len(y3)+1), y4, label='ASGD Weight-Dropped LSTM')
    plt.loglog(np.arange(1,len(y2)+1), y3, label='Recurrent Highway Network')
    plt.loglog(np.arange(1,len(y4)+1), y5, label='Transformer-XL')
